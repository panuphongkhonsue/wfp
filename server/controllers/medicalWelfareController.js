...
const { reimbursementsGeneral, categories, subCategories, reimbursementsGeneralHasSubCategories, users, positions, sector, employeeTypes, departments, sequelize } = require('../models/mariadb');
    ...
    create = async (req, res, next) => {
        ...
        try {
            const result = await sequelize.transaction(async t => {
                const newItem = await reimbursementsGeneral.create(dataCreate, { transaction: t, });
                var itemsReturned = {
                    ...newItem.toJSON(),
                };
                if (selectedAccident) {
    ...
    update = async (req, res, next) => {
        ...
        try {
            const result = await sequelize.transaction(async t => {
                ...
                if (selectedAccident) {
                    const existingAccident = await reimbursementsGeneralHasSubCategories.count({
                        where: { reimbursements_general_id: dataId, sub_categories_id: 1 },
                        transaction: t,
                    });
                    if (!existingAccident) {
                        const newItemSub = await reimbursementsGeneralHasSubCategories.create(
                    ...
                    const deleteItemSub = await reimbursementsGeneralHasSubCategories.destroy(
                        {
                            where: { reimbursements_general_id: dataId, sub_categories_id: 1 },
                            transaction: t,
                        }
                ...
                if (selectedPatientVisit) {
                    const existingPatientVisit = await reimbursementsGeneralHasSubCategories.count({
                        where: { reimbursements_general_id: dataId, sub_categories_id: 2 },
                        transaction: t,
                    });
